...
%Converting target output to 2 output neurons
for i=1:length(Y)
    if Y(i)==1
        z(i,:) = [1 0 0];
    else
...
p = randperm(length(Y));
trainInput = X(p(1:(0.7*size(X,1))),:); 
trainOutput = z(p(1:(0.7*size(X,1))),:);
testInput = X(p(0.7*size(X,1)+1:end),:); 
testOutput = z(p(0.7*size(X,1)+1:end),:);
k = 1000; %Hidden neurons
