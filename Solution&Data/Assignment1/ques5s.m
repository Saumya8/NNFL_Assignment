...
k = input('Enter number of iterations: '); %In this case, 200
m = 25; %Number of training instances taken in each iteration
...
for i=1:k
    ...
    for j=1:m
        ...
        for b = 1:3
        w(b) = w(b) - alpha*(h'*X(a(j),b))-0.5*alpha*lambda*sign(w(b)); %Weight updates
        W((i-1)*m+j+1,b) = w(b); %Storing weights in array
        end
        J((i-1)*m+j+1) = evaluatecostfunction(X,y,w,lambda); %Calculating cost
    end
end
...
figure; plot3(W(:,3),W(:,2),J,'color','r'); grid ON; %3D plot
