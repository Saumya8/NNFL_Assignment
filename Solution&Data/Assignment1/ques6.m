clc;
X = xlsread('D:\CMS\Sem3-1\Neural Networkand Fuzzy Logic\data2.xlsx');
...
classColors = zeros(size(X,1), 3); %Array to assign class colors to each data element, (3 for r,g,b)
...
function [v, c] = kMeansClustering(X,K,n)
    ...
    end %random cluster centre are alloted
    for i = 1:n
        d = zeros(row,K); %Array to store distances
        for j=1:row
            for k = 1:K
                %the distance between cluster center and the data point
